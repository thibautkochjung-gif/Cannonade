...
func _populate_contracts() -> void:
	for contract in available_contracts:
		...
		button.pressed.connect(func(): GameManagerScene.start_contract(contract))
...
func _populate_loadout() -> void:
	...
	for slot_type in ModuleData.SlotType.values():
		...
		for slot_index in equipped.size():
			loadout_list.add_child(_build_slot_row(slot_type, slot_name, slot_index, equipped[slot_index]))


func _build_slot_row(slot_type: ModuleData.SlotType, slot_name: String, slot_index: int, module: ModuleData) -> HBoxContainer:
	var row := HBoxContainer.new()

	var label := Label.new()
	label.text = "%s: %s" % [slot_name, module.display_name if module else "(empty)"]
